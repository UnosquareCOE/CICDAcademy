        ...
        stage('Deploy Database') {
            ...
            steps {
                script {
                    ...
                    docker.image('flyway/flyway').withRun('-v "$WORKSPACE/database:/flyway/sql"', '-url=jdbc:postgresql://${DB_URL}/cicdtestdb -schemas=public -user=${DB_USER} -password=${DB_PASSWORD} -locations=/flyway/sql -connectRetries=5 migrate') { c ->
