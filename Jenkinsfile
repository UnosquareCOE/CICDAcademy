        ...
        stage('Deploy Database') {
            ...
            steps {
                script {
                    ...
                    docker.image('flyway/flyway').withRun('-v "${PWD}/database:/flyway/flyway/sql"', '-url=jdbc:postgresql://${DB_URL}/cicdtestdb -schemas=public -user=${DB_USER} -password=${DB_PASSWORD} -connectRetries=5 migrate') { c ->
                        sh "docker exec ${c.id} ls /flyway/flyway/sql"
